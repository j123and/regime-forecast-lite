...
def rmse(y_true: Sequence[float], y_pred: Sequence[float]) -> float:
    return math.sqrt(sum((a - b) ** 2 for a, b in zip(y_true, y_pred, strict=False)) / max(1, len(y_true)))
...
def detection_metrics(
    ...
    if not true_flags or sum(int(f) for f in true_flags) == 0:
        out.update(
            {
