...
def main() -> None:
    ...
    ap.add_argument(
        "--max_rows", type=int, default=None, help="Optional cap on rows for faster sweeps"
    )
    ...
    # progress impl: per combo (import to a separate name, store in typed holder)
    tqdm_call: Callable[..., Any] | None
    try:
        from tqdm import tqdm as _tqdm_imported

        tqdm_call = _tqdm_imported
    except Exception:
        tqdm_call = None
    ...
    try:
        with ProcessPoolExecutor(max_workers=max(1, args.jobs)) as ex:
            futs = [
                ex.submit(_run_one, data_path, args.alpha, args.cp_tol, args.max_rows, c)
                for c in combos
            ]

            if args.progress and tqdm_call is not None:
                pbar = tqdm_call(total=total, unit="combo", smoothing=0.05)
